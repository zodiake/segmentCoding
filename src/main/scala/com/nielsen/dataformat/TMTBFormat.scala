...
object TMTBFormat {
  def main(args: Array[String]): Unit = {
   ...
   // conf.setMaster("local")
    ...
    val descFormat = desc.map{x=>(x,x.split(",",-1))}.filter(_._2.length >= 20).map{x=>(x._1,x._2(4)+","+x._2(5)+","+x._2(6),x._1.indexOf(x._2(4)+","+x._2(5)+","+x._2(6)))}
                     .map{x=>(x._1.substring(0, x._3),formatFunc(x._2,configMap),x._1.substring(x._3+x._2.length()))}.map(x=>x._1+x._2+x._3)
