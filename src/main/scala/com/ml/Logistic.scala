...
object Logistic {
  def main(args: Array[String]): Unit = {
    ...
    val result = formatedRDD.zip(values).map {
      case (l1, l2) => l1.zip(l2)
    }
    val t = result.map(i => if (!i.isEmpty) i.maxBy(_._2) else ("", 0.0)).filter(_._2 == 0.0)

    val c = t.aggregate(List[(String, Double)]())((u, v) => v :: u, (u, v) => u ++ v)
