...
object TotalnotImprove {
    ...
    for (path <- raw_data_path) { //-----------??????????????
      ...
      for (month <- months) {
        ...
        for (catcode <- catlist) {
          ...
          val tempre = testFile.map(x => coding(x, configFile, seglist, subbrand_namelist, args(5), KRAFile, cateCodeCombine, segNoCombine, brand_conf, configFileNew))
  ...
  /*
   * @param catcode: category分类
   * @param itemfile: segment config file
   */
  def itemmaster_brand(catcode: String, itemfile: List[Array[String]]): List[List[(String, String)]] = {
    ...
    val result = List(eccbrandlist, eccshortdesc, eccmanulist, ecbrandlist, ecmanulist, parentidlist)
    mapCache += catcode -> result
    return result
  }
  ...
  def coding(item_raw: Array[String], configFile: List[Array[String]],
             seglist: List[String], subbrand_namelist: List[String], codingType: String,
             krafile: List[Array[String]], cateCodeCombine: String, segCombine: String, brand_conf: List[List[(String, String)]], configFileNew: List[Array[String]]): (item, List[String]) = {
  ...
  //提取item.split(",")(0)变量
  /*
   * @param item:某条需要coding的记录
   * @param cateConf:category config file
   */
  ...
  def transCateCode1(item: String, cateConf: Broadcast[Map[String, String]]): String = {
    val head = item.split(",")(0)
    val cateTrans = cateConf.value.get(head.toUpperCase())
